...
const Topics = (props: { posts: TopicsPropType[] }) => {
  ...
  const leftSlideStyle: string[] = [
    `w-[${leftSlideRatio.frame}vw]`,
    `h-[${leftSlideRatio.frame}vh]`,
    `translate-x-[${leftSlideRatio.translateX}]`,
    `-translate-y-[${(leftSlideRatio.frame - centerSlideRatio.frame) / 2}vh]`,
  ]
  const centerSlideStyle: string[] = [
    `w-[${centerSlideRatio.frame}vw]`,
    `h-[${centerSlideRatio.frame}vh]`,
    `translate-x-[${centerSlideRatio.translateX}]`,
    'center',
  ]
  const rightSlideStyle: string[] = [
    `w-[${rightSlideRatio.frame}vw]`,
    `h-[${rightSlideRatio.frame}vh]`,
    `translate-x-[${rightSlideRatio.translateX}]`,
    `-translate-y-[${(leftSlideRatio.frame - centerSlideRatio.frame) / 2}vh]`,
  ]
  const leftElseSlideStyle: string[] = [
    `w-[${leftSlideRatio.frame}vw]`,
    `h-[${leftSlideRatio.frame}vh]`,
    'translate-x-[-100vw]',
    `-translate-y-[${(leftSlideRatio.frame - centerSlideRatio.frame) / 2}vh]`,
  ]
  const rightElseSlideStyle: string[] = [
    `w-[${leftSlideRatio.frame}vw]`,
    `h-[${leftSlideRatio.frame}vh]`,
    'translate-x-[100vw]',
    `-translate-y-[${(leftSlideRatio.frame - centerSlideRatio.frame) / 2}vh]`,
  ...
  const getSlideStyle = (index: number): string[] => {
    if (index == 0) return leftSlideStyle
    else if (index == 1) return centerSlideStyle
    else if (index == 2) return rightSlideStyle
  ...
  return (
    <section className='w-[100vw]'>
      <h1
        className={`topics-title mb-3 underline ${madimiOne.className} px-[3vw] py-[1vh]`}
      >
        topics
      </h1>
      ...
      <div className='m-3 flex flex-row justify-center items-center'>
        <button
          onClick={slideRight}
          className={`border-r-[length:--topics-arrow-size] border-r-white border-b-[length:--topics-arrow-size] border-b-transparent border-t-[length:--topics-arrow-size] border-t-transparent`}
        ></button>
        <div className='space-x-3 rounded-3xl w-36 h-5 m-2 flex flex-row justify-center items-center'>
          {Array.from({ length: 5 }).map((_, index) => (
            <div
              key={index}
              className={`point h-3 w-3 rounded-full ${index == 1 ? focusedPointStyle.join(' ') : normalPointStyle.join(' ')}`}
            ></div>
          ))}
        </div>
        <button
          onClick={slideLeft}
          className={`border-l-[length:--topics-arrow-size] border-l-white border-b-[length:--topics-arrow-size] border-b-transparent border-t-[length:--topics-arrow-size] border-t-transparent`}
        ></button>
      </div>
    </section>
  )
