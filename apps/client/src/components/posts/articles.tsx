...
const Article = (props: ArticlePropType) => {
  const trends: string[] = ['new', 'popular', 'hot']
      ...
      >
        <div className='flex flex-row items-end'>
          ...
          <div className='text-xl space-x-3 mx-3'>
            {props.categoryList.map((category, index) => (
              <button
                key={index}
                className={`px-3 text-3xl cursor-pointer group ${madimiOne.className}`}
        ...
        <div>
          {trends.map((trend, index) => (
            <button
              key={index}
              className={`px-3 text-xl group ${madimiOne.className}`}
