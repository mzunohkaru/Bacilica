...
const Initial: React.FC = () => {
  ...
  return (
    <div
      ...
      className='w-screen h-screen font-[family-name:var(--font-geist-sans)] bg-[#0B0B0B]'
    >
      <main className='flex flex-col flex-grow w-full h-full justify-between text-white z-10'>
        ...
        {/*----------------------*/}
        <section
          className={`flex flex-row h-[10vh] justify-center w-full items-center z-10 text-4xl font-leckerli bg-black ${leckerli.className}`}
        >
          <button onClick={handleLoginModal} className='flex-grow'>
            log in
          </button>
          <div className='h-[70%] border border-white'></div>
          <button onClick={handleSignUpModal} className='flex-grow'>
            sign up
          </button>
        </section>
      </main>
